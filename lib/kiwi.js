...
Kiwi.fn = Kiwi.prototype = {
  m: {},
  v: {},
  c: {},
  base_url: '',
  default_error_handler: function(message, error_code){}
};

//Connector Object
Kiwi.Connector = Kiwi.fn.Connector = function(model){
  this.model = model;  
};
...
Kiwi.Connector.prototype = {
  ...
  http_request: function(uri, method, params, after_success, after_error, options){
    ...
    params = params || {}; 
    //From the scope
    var request = typeof __request__ == 'undefined' ? false : __request__;
    if (!uri) {uri = Kiwi.fn.base_url + this.uri();}  
    options = $.extend({
      type: method,
...
Kiwi.ServerRequestHandler = Kiwi.fn.ServerRequestHandler = function(success_handler, error_handler){
  //Success handler is always passed, error handler is an object with a map of http error codes to functions or a function under the key 'all'
...
Kiwi.ServerRequestHandler.prototype = {
  error: function(xhr, status_message){
   ...
   var message_object = {};
    ...
    var args = [xhr.status, message_object];
       
    //Call custom universal error handler if passed
    if (this.error_handler.all){
      this.error_handler.all.apply(this, args);
      return;
    }    
    //Call custom by-error-code handler if passed
    if(this.error_handler[xhr.status]){
      this.error_handler[xhr.status].apply(this, args);
      return;
    }        
    //Call default error handler
    Kiwi.default_error_handler.apply(this, args);
  }
};
...
Kiwi.Parser = {
  get_event_name: function(string){
    return strinKiwi.substring(strinKiwi.lastIndexOf(" ") + 1);
  },
  get_selector: function(string){
    return strinKiwi.substring(0, strinKiwi.lastIndexOf(" "));
  },
  get_handler: function(string, object){
    ...
    if (strinKiwi.indexOf(".") != -1){
      var a = strinKiwi.split(".");
      var controller = a[0];
      var action = a[1];
...
//Request object -- handles persistence through action chain, is temporarily assigned to a global variable when action is being executed
Kiwi.Request = Kiwi.fn.Request = function(event, view, options){
  this.event = event;
  this.view = view;
  this.options = options;
  this.data = {};
  this.handler = Kiwi.Parser.get_handler(this.options.action, this.view);
  this.catch_errors = options.on_error;
...
//Model
Kiwi.m = Kiwi.fn.m = function(name, model){
  var new_model = Object.create(Kiwi.m.prototype);
  new_model.name = name;
  new_model.connector = new Kiwi.Connector();
  Kiwi.m[name] = $.extend(new_model, model);
};

// Model methods
Kiwi.m.prototype = Kiwi.fn.m.prototype = {
  name: null,
  type: 'model',
  connector: null,
  has: {},
  ...
  id: function(id){
    ...
    $.each(model.has, function(key, name){
      var related_model = $.extend({}, Kiwi.m[name], {parent: model});
...
Kiwi.v.prototype = Kiwi.fn.v.prototype = {
  ...
  register_listeners: function(){
    ...
    $.each(this.listeners, function(key, value){
      ...
      value = $.extend({callback: 'update', subscribe: true, on_error: null, show_loader: true, unbind: true}, value);
      
      //General callback
      var callback = function(event){               
        //__request__ :an implied global, it is accessible within the called action, also accessible with 'this'
        __request__ = new Kiwi.Request(event, view, value);
        
        __request__.execute_action();        
        
        //If request has ajax call cleanup is delegated to post ajax request routine
        if (__request__.current_ajax_calls === 0) {__request__.clean_up();}        
