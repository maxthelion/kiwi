...
Kiwi.Connector.prototype = {
  ...
  http_request: function(uri, method, params, after_success, after_error, options){
    ...
    if (request){request.query_params = params;}
    ...
    switch(this.typeOf(value)){
      case 'array':
        for (var i = 0; i < value.length; i++){
          var element = this.serialize(value[i], prefix+'[]')
      ...
      case 'object':
        for (key in value){
          var element = this.serialize(value[key], prefix ? prefix+'['+encodeURIComponent(key)+']' : encodeURIComponent(key))
          if (element !== null){retval_array.push(element);} 
        }
        retval = retval_array.length ? retval_array.join("&") : null;
        break;
