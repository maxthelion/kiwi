...
Kiwi.Connector.prototype = {
  ...
  serialize: function(params){
    return Kiwi.Parser.serialize(params);
  },
  ...
  error: function(xhr, status_message){
   ...
   var message_object = {};
    ...
    switch (xhr.status){
      ...
      case 0:
        message_object.error = "Could not contact the server, please make sure you are connected to the Internet.";
        break;
      case 200:
...
//Parser object
Kiwi.Parser = {
  //Use with caution -- returns 'object' if array is zero length
  typeOf: function(obj) {
  ...
  serialize : function(value, prefix) {
    var retval = [];
    var prefix = prefix || '';
    switch(this.typeOf(value)){
      case 'array':
        for (var i = 0; i < value.length; i++){
          retval.push(this.serialize(value[i], prefix+'[]'));
        }
        retval = retval.join("&");
        break;
      case 'object':
        for (key in value){
          retval.push(this.serialize(value[key], prefix ? prefix+'['+key+']' : key));
        }
        retval = retval.join("&");
        break;
      default:
...
Kiwi.Request = function(listened, event, view, options){
  var that = this;
  this.event = event;
  this.view = view;
...
Kiwi.v.prototype = {
  ...
  register_listeners: function(){
    ...
    $.each(this.listeners, function(key, value){  
      ...
      $.each(value, function(index, options){
        ...
        view.bind(full_selector, event_name, callback, callback.options.live);
      })
    });
  },
  __general_callback: function(options){
    ...
    var callback = function(event){       
      ...
      Kiwi.__request__ = new Kiwi.Request(this, event, view, options);
  ...
  trigger_request_with: function(event, options){
    this.__general_callback(options).call(this, event)
  },
  ...
  bind: function(full_selector, event_name, callback, live) {
    live ? $(full_selector).live(event_name, callback) : $(full_selector).bind(event_name, callback)
  },
...
Kiwi.helpers.prototype = {
  type: 'helper',
  render_with: function(template, data, exclude, strict_recurse) {
    ...
    data = data || {};
    exclude = exclude || [];
    var insert = function(data, parents){
      parents = parents || []
      $.each(data, function(key, value){
        if (value !== null && Kiwi.Parser.typeOf(value) === 'object') {
          new_parents = strict_recurse ? parents.concat("." + key) : []
          insert(value, new_parents)
        } else {
          if ($.inArray(key, exclude) != -1){return;}
          var selector = parents.join(" ") + ' .'+ key
