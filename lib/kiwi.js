...
Kiwi.Parser = {
  ...
  get_handler: function(string, object){
    ...
    //If a period is encountered assume a controller and get action, if not assume current view/controller or helper
    if (string.indexOf(".") != -1){
      var a = string.split(".");
      var controller = a[0];
      var action = a[1];
    ...
    try{
      ...
      if (this.handler.klass.type === 'view' || this.handler.klass.type === 'helper') {        
        ...
        this.handler.action.apply(this.view, [this]);
         ...
         //Before filter
         if (this.handler.klass.before){
           var filter = this.handler.klass.before
           if (Kiwi.Parser.typeOf(filter) == 'array'){
             if ($.inArray(this.handler.action_name, filter[0]) != -1){this.handler.klass[filter[1]].apply(this)}
           } else {
             this.handler.klass[filter].apply(this)
           }
         }         
...
Kiwi.v.prototype = {
  ...
  register_listeners: function(){
    ...
    $.each(this.listeners, function(key, value){
      ...
      value = $.extend({callback: 'update', subscribe: true, on_error: null, show_loader: true, unbind: true, prevent_default: true}, value);
      ...
      var callback = function(event){               
        ...
        if (value.prevent_default) {return false;}        
      }; 
  ...
  initialize_partials: function(){
    ...
    $.each(view.node.next('.partials').children(), function(){
      ...
      var template = view.partials[($(this)[0].className)] = $(this).clone();
